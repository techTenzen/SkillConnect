...
export function registerRoutes(app: Express): Server {
  ...
  // Add this to your routes.ts file - it will help us debug
  app.get("/api/debug/invitations", async (req, res) => {
    ...
    let allInvitations;
    try {
      // If using DrizzleStorage
      allInvitations = await db.select().from(invitations);
    } catch (error) {
      // If that fails, try TestStorage
  ...
  app.post("/api/connection-requests", async (req, res) => {
    ...
    try {
      ...
      if (alreadySent) {
        // Return a special status code that frontend can interpret as "already connected"
        return res.status(409).json({
